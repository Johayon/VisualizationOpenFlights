...
int main(int argc, char ** argv)
{
	...
	country.erase(sz(country) - 1);
	for (int i = 0; i < sz(country); ++i)
	...
	for (vector <ROUTE> :: iterator i = routes.begin(); i != routes.end(); ++i)
	{
		...
		if ((airports[index_src].country == country && airports[index_dest].country != country) ||
			(airports[index_src].country != country && airports[index_dest].country == country))
		{
			cout << i->id << ',';
